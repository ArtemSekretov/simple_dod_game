...
enemy_instances_spawn(EnemyInstancesUpdateContext *context)
{
    ...
    while (enemy_instances_update->EnemyPositionsCount < wave_instance.EnemyInstancesCount)
    {
        u16 wave_instance_index = enemy_instances_update->EnemyPositionsCount;
        ...
        enemy_instances_update->EnemyPositionsCount++;
    }
}

static void
enemy_instances_move(EnemyInstancesUpdateContext *context, f32 delta)
{
    EnemyInstances *enemy_instances              = context->EnemyInstancesBin;
    EnemyInstancesUpdate *enemy_instances_update = context->Root;
...
enemy_instances_next_wave(EnemyInstancesUpdateContext *context)
{
    ...
    if (enemy_instances_update->EnemyPositionsCount != wave_instance.EnemyInstancesCount)
    {
        return;
    }
    ...
    enemy_instances_update->EnemyPositionsCount = 0;
}

static void
enemy_instances_update(EnemyInstancesUpdateContext *context, f32 delta)
{  
    EnemyInstances *enemy_instances              = context->EnemyInstancesBin;
    EnemyInstancesUpdate *enemy_instances_update = context->Root;
    ...
    enemy_instances_move(context, delta);

    enemy_instances_update->WaveTime += delta;
